...
???+ info "X 分音"
    ...
    在此基础上，每增加一根符尾，其时值便缩短一半，其名称亦为对应的 X 分音符。如八分音符、十六分分音符等。
...
当音乐存在节拍变化时，你需要调整页长，每拍对应一倍的 TimeBase（默认值 480），使得一页内的拍数与音乐的小节对应。
...
变速提示的默认位置是线速变化两拍之前（也就是 480 Tick 之前）。如果你想要自己放置变速提示，请遵守这一规范。请不要在没有发生对应速度变化的地方使用变速提示，追求演出效果可以使用自定义事件。
